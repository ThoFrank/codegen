...
impl Module {
    ...
    /// removes the scope block
    pub fn without_scope(&mut self) -> &mut Self {
        self.scope = None;
        self
    }
    ...
    pub fn import(&mut self, path: &str, ty: &str) -> &mut Self {
        self.scope.get_or_insert_with(Scope::new).import(path, ty);
        self
    }
    ...
    pub fn new_module(&mut self, name: &str) -> &mut Module {
        self.scope.get_or_insert_with(Scope::new).new_module(name)
    }
    ...
    where
        String: PartialEq<Q>,
    {
        self.scope.as_ref().and_then(|s| s.get_module(name))
    }
    ...
    where
        String: PartialEq<Q>,
    {
        self.scope.get_or_insert_with(Scope::new).get_module_mut(name)
    }

    /// Returns a mutable reference to a module, creating it if it does
    /// not exist.
    pub fn get_or_new_module(&mut self, name: &str) -> &mut Module {
        self.scope.get_or_insert_with(Scope::new).get_or_new_module(name)
    }
    ...
    pub fn fmt(&self, fmt: &mut Formatter<'_>) -> fmt::Result {
        ...
        match self.scope{
            Some(ref s) => fmt.block(|fmt| s.fmt(fmt)),
            None => write!(fmt, ";")
        }
